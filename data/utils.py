...
def verify_image_label(args):
    ...
    try:
            ...
            if nl:
                if keypoint:
                   ...
                   # assert (lb[:, 6::ndim] <= 1).all(), 'non-normalized or out of bounds coordinate labels'
                    ...
                    assert (lb[:, 1:15] <= 1).all(), \
                        f'non-normalized or out of bounds coordinates {lb[:, 1:15][lb[:, 1:15] > 1]}'
                    assert (lb[:, 1:15] >= 0).all(), f'negative label values {lb[lb[:, 1:15] < 0]}'
                # All labels
        ...
        lb = np.concatenate((lb[:, :5], lb[:, 15:18]), axis=1)
