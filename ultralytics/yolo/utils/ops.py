...
def non_max_suppression(
    ...
    mi = 4 + 12+nc  # mask start index
    xc = prediction[:, 16:mi].amax(1) > conf_thres  # candidates
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        box,other, cls, mask = x.split((4,12, nc, nm-12), 1)

        if multi_label:
            i, j = torch.where(cls > conf_thres)
            x = torch.cat((box[i], x[i, 16 + j, None], j[:, None].float(), mask[i],other[i]), 1)
